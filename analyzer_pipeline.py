...
⭐ 한국어 작문 원칙:
- 모든 문장은 명확한 주술 구조를 가져야 함 (주어-서술어 명확)
- 전문적이고 정중한 높임말 사용 (주장이 아닌 객관적 서술)
- 문법 오류 절대 금지 (문장 검토 후 작성)
- 각 섹션은 반드시 "근거 → 평가 → 점수 반영" 순서로 작성

⭐ 구체적 작성 예시:

【우발채무 & 소송】
구체적 근거: {corp_name}은 공시 주석 [◯절. 우발부채]에 따르면 소송 사건이 ◯건입니다.
평가: 이는 [업계 평균 대비/역사적으로] [높음/낮음/중간]입니다.
      따라서 법적 분쟁 리스크가 [높다/낮다/보통]로 평가됩니다.
점수 반영: 위 사항을 종합하여 우발채무 카테고리에서 ◯점을 부여합니다.

【특수관계자거래】
구체적 근거: {corp_name}은 공시 주석 [◯절. 특수관계자거래]에 따르면
           지배주주/계열사와의 거래 규모가 총 ◯◯억원입니다.
평가: 이는 전체 매출액의 ◯%를 차지하며, 업계 평균 ◯%보다 [높음/낮음/비슷함]합니다.
      경영진의 자산 이전 신호가 [강함/약함/없음]으로 평가됩니다.
점수 반영: 위 사항을 종합하여 특수관계자거래 카테고리에서 ◯점을 부여합니다.

【자산손상 & 투자손실】
구체적 근거: {corp_name}은 공시 주석 [◯절. 자산손상차손]에서
           ◯년도에 ◯◯억원의 자산손상을 인식했습니다.
평가: 이는 [기존 관례/급격한 변화]를 나타내며,
     [종속기업 투자/무형자산/부동산] 가치 하락 신호로 평가됩니다.
점수 반영: 위 사항을 종합하여 자산손상 카테고리에서 ◯점을 부여합니다.

【최종 종합 평가】
정성 분석 종합: 위 3가지 카테고리 분석 결과를 총합하면,
               {corp_name}의 공시 주석상 정성 리스크는 [낮음/중간/높음] 수준입니다.
투자 적격성 판단: [강력 추천 / 조건부 권고 / 경고 / 진입 금지]
